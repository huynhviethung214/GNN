...
if __name__ == '__main__':
    inputShape = [28, 28]
    nInputs0 = inputShape[0] * inputShape[1]
    ...
    if isTrain:
        ...
        #                  colour='green'):
        #     v = v - 1
        #     if v < evaluatingClasses.shape[0]:
        #         # Target output vector v
        #         t = np.zeros((nOutputs0,), dtype=np.float64)
        #         t[v] = 1
        #
    ...
    network0 = Network(
        ...
        epochs=EPOCHS, datasetCap=datasetCap, frequency=5,
        minRadius=3000, maxRadius=5000,
        etaW=1e-0, etaB=1e-0, etaP=1e-0, etaR=1e-0, decay=0.1,
        width=8000, height=8000, depth=8000, minBias=0.1, maxBias=0.8,
        hiddenZoneOffset=1500, outputZoneOffset=1500,
        ...
        activationFunc='sigmoid', datasetName='mnist', train=isTrain
    )

    network1 = Network(
        nInputs=nOutputs0, nHiddens=nHiddens1, nOutputs=nOutputs1,
        epochs=0, datasetCap=0, frequency=5,
        ...
        activationFunc='tanh', datasetName='mnist', train=isTrain
    )

    # network2 = Network(
    #     nInputs=nOutputs0 + nOutputs1, nHiddens=80, nOutputs=6,
    ...
    #     maxInputPerNode=maxInputPerNode1, minInputPerNode=maxInputPerNode1 // 2,
    #     maxOutputPerNode=maxOutputPerNode1, minOutputPerNode=maxOutputPerNode1 // 2,
    #     activationFunc='tanh', datasetName='emnist', train=isTrain
    # )

    if isTrain:
        folderName = get_foldername()
        print(f'Using Folder\'s Name: {folderName}')
        ...
        #
        # # print(f'Prediction: {labels[int(np.argmax(res))]} '
        # #       f'with confidence: {res[np.argmax(res)] * 100. / nTest}%')
